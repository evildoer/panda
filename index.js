...
function dbBackupTick ()
{
    for (const _srv of dbServerListOn ())
    {
        ...
        const _res = dbBackupRefresh (_file, _bak, _cur.rows, _old);
        if (!_res.ok)
            console.error ('[' + (d()) + '] [backup] плановая копия ' + _name + ' не обновилась: ' + _res.why);
...
function queueView (m, start, moveSel = 0, opts = {})
{
    ...
    // а не №0». Поэтому сверху остаётся только рабочее (сеть, пачки, перенос), затем
    // очередь, а цифры (по авторам, до конца, проверка заранее) -- в конце, над кнопками
    // (кнопки Discord всегда рисует ПОД embed'ом, ближе к низу их не поставить).
