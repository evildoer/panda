...
// НУМЕРАЦИЯ: vX.YY -- метка ПАРТИИ изменений, а не счётчик версий. Пропусков (например
// v2.76, v2.77) искать не надо: если номера нет ни в одном блоке, значит партии с таким
...
//     остаётся только «сеть/прокси молчит: стучусь...» -- эта строка появляется, лишь
//     когда музыка уже встала, и объясняет, что происходит. Маршрут по-прежнему видит
//     ТОЛЬКО владелец: проверка внутри netRouteText, обойти её нельзя.
//   * ЧЕСТНО ПРО ОБРЫВ ПОТОКА. «выравнивание громкости оборвалось: ... -- звук этого
//     трека оборвался: до конца он не доиграет, очередь продолжаю со следующего»
//     читалось как «виновата громкость» и «трек потерян» (владелец: «звучит фатально,
//     даже правдой не оказалось»). Теперь там сказано прямо: loudnorm -- ПОСЛЕДНИЙ
//     в цепочке yt-dlp -> ffmpeg и ругается он на обрыв ПОТОКА (обычно из-за сети),
//     а трек не бросают -- его ещё попробуют продолжить с той же секунды (до
//     MUSIC_STREAM_RETRIES раз); про пропуск говорит только строка «поток обрывается
//     снова», и только она.
//   * ПЛАНОВЫЕ КОПИИ ВИДНО ВСЕГДА: строка про каждый сервер на КАЖДОМ проходе --
//     «обновлена (9 -> 10)», «обновлена (без изменений, 9)», «в базе 0 записей»,
//     «базы ещё нет», «база ПОВРЕЖДЕНА», «не обновилась: причина». Плюс строка
//     «прошлый проход был X назад», если таймер простоял (например, ПК спал).
// CHANGELOG v2.91 (сводка /queue -- вниз; живых сообщений несколько; копии видно в логе):
//   * СВОДКА /QUEUE ПЕРЕЕХАЛА В САМЫЙ НИЗ сообщения: «По авторам», «До конца очереди»
//     и «Проверка очереди заранее» стоят ПОСЛЕ списка очереди (перед кнопками), а не
//     между «Сейчас» и очередью (владелец: «этот блок в САМЫЙ НИЗ, он разрывает контроль
//     и работу с очередью; я постоянно думаю, что играет №1, а не №0»). Сверху остаётся
//     рабочее: состояние сети, строка пачек и подпись переноса.
//   * ХВОСТ «...и ещё N» -- БЕЗ «/queue from:N»: помощь по команде в конце списка не
//     нужна, на следующую страницу ведут кнопки листания.
//   * ЖИВЫХ СООБЩЕНИЙ /QUEUE МОЖЕТ БЫТЬ НЕСКОЛЬКО (до QUEUE_LIVE_MAX = 4). Раньше живая
//     запись была ровно одна, поэтому второе открытое /queue (другой аккаунт, другой
//     канал) вытесняло первое -- и оно замирало до ручного «🔄 Обновить» (владелец:
//     «переключается только»). Теперь у каждого сообщения своя запись (канал, страница,
//     зритель, время правки), и обновляются все -- и самообновлением, и после действий.
//   * СБОЙ ПРАВКИ БОЛЬШЕ НЕ УБИВАЕТ ЖИВОЕ СООБЩЕНИЕ: сетевой таймаут (в логе владельца --
//     «Connect Timeout Error») считался поводом забыть сообщение, и оно оставалось
//     замороженным. Теперь временная беда маршрута повторяется на следующем проходе,
//     а запись снимается только там, где вести бессмысленно (10008 -- сообщение удалили,
//     403/50001 -- нет прав); про ошибку -- одна строка в лог на серию, без спама.
//   * ПЛАНОВАЯ КОПИЯ ПИШЕТ В ЛОГ КАЖДЫЙ ПРОХОД (и «обновлена», и «без изменений», и
//     сбой с причиной): раньше строка появлялась только когда менялось число записей, и
//     по логу нельзя было отличить «таймер работает» от «таймер молчит» (владелец:
//     «строчка появляется НЕ РЕГУЛЯРНО»: в логе стояло 17:17 и 20:17 -- и ничего между).
...
//     Плейлист читался рвано, а 0-го элемента (того, что играет) вообще не было ни в
//     нумерации, ни в /move.
//   * ПАЧКА -- ЭТО ПОДРЯД ИДУЩИЕ ТРЕКИ ОДНОГО АВТОРА, А ИГРАЮЩИЙ ТРЕК -- ЕЁ ГОЛОВА.
//     Полный плейлист теперь читается как №0 (играет сейчас) + №1..№N (очередь), и треки
//     одного автора идут в нём подряд -- ни /play, ни /push, ни перестановка, ни переход
//     этого не ломают. Если играет трек Фантазёра, то и остальные его треки стоят сразу
//     за ним, а не через чужие (владелец: «Нельзя допускать разрыва пачек... Играемый
//     трек считается 0-вым элементом, позицией в плейлисте и в пачке!»).
//   * /MOVE ПОНИМАЕТ 0: `to:0` -- этот трек играет сразу (старый играющий возвращается
//     в очередь, со своей секундой), `number:0` -- снять играющий с эфира (трек встаёт
//     в НАЧАЛО СВОЕЙ пачки, играет следующий). В интерфейсе очереди -- кнопка
//     «▶ Играющим» в режиме переноса и №0 в окне «На позицию…».
//   * ОТКАЗ ВМЕСТО РАЗРЫВА: если трек встал бы ВНУТРЬ чужой пачки, команда не молчит и
//     не ломает плейлист, а говорит, что именно произойдёт и как сделать иначе (`/push`).
//   * /PUSH СТАВИТ ПОСЛЕ ИГРАЮЩЕГО, А НЕ ПЕРЕД НИМ (раньше он и рвал пачку автора этого
//     трека), а если пачку своего автора когда-то разорвало -- эта же команда её склеит.
//   * /JUMP («срочный переход») -- теперь та же операция, что «▶ Играющим»: одна логика
//     на слэш-команду, кнопки и /move, поэтому поведение не может разъехаться.
// CHANGELOG v2.88 (политики в репозитории нет; подсказки -- из примера):
//   * PRIVACY.md -- ЛИЧНЫЙ ФАЙЛ, А НЕ ФАЙЛ РЕПОЗИТОРИЯ (решение владельца): в нём ник
//     владельца, названия ЕГО серверов, сроки из ЕГО config.json и ЕГО значок ключа.
//     Файл убран из git и внесён в .gitignore; кто ставит бота себе -- собирает свою
//     политику той же командой `node . privacy` из шаблона privacy.template.md.
//   * ПОДСКАЗКИ К КЛЮЧАМ ЭТАЛОННЫ -- В ПРИМЕРЕ: если у ключа появилось новое поведение,
//     сначала правится `_comment` в config.example.json, а потом та же подсказка
//     копируется в остальные конфиги (в боевой config.json в первую очередь).
...
//     то есть любой свежий бот писал свою строку в шапку канала и затирал текст АВТОРА
//     канала -- а вернуть его нечем (роут у Discord только пишущий). Лишнее поведение
//     включается одним ключом, и в примере он стоит явно.
//   * Строка [music] при старте больше не говорит «channel_status: false» там, где ключа
//     нет вообще: теперь у неё два разных ответа (ключ или умолчание), как и в `node .
//     config`. Мусор в ключе ("да", 1) -- это «не трогаю»: опечатка не должна затирать
//     чужой текст в шапке -- об этом говорит строка [config].
...
//     если канала из прошлого запуска уже нет (или в нём пусто), checkListeners больше не
//     отсиживается в return, а ищет СЛУШАТЕЛЯ -- автора ближайшего нужного трека
//     (pickAuthorChannel) и едет туда; никого нет -- ждёт /join или следующего захода.
...
// [v2.91] Живые сообщения /queue -- для дампа. Их может быть несколько (каждое -- своя
// страница и свой зритель), поэтому показываем все (первые три, дальше счётчиком).
...
function dbDumpFmt (_ns, _key, _value)
{
    ...
    if (_ns === 'musicState')
    {
        ...
        return _head + ' -- в очереди ' + _tr.length +
            ...
            // [v2.57] ...и видно, ЧТО именно бот ведёт (сообщения /queue обновляются сами):
            ...
            // [v2.91] Таких сообщений может быть несколько -- показываем все.
...
// [v2.88] PRIVACY.md -- ЛИЧНЫЙ ФАЙЛ, А НЕ ФАЙЛ РЕПОЗИТОРИЯ (решение владельца): в нём
// ник владельца, названия ЕГО серверов, сроки из ЕГО config.json и ЕГО значок ключа.
// Поэтому он в .gitignore и из репозитория убран: кто ставит бота себе -- генерирует
// свою политику этой же командой (шаблон privacy.template.md в репозитории остаётся),
// а свою выкладывает по адресу из privacy_url (например в gist).
// Аргументы:
...
// 0 -- как было: только при старте).
// [v2.91] В ЛОГ ПИШЕМ КАЖДЫЙ ПРОХОД. Раньше строка появлялась только когда число
// записей изменилось, и владелец справедливо решил, что копии делаются «когда придётся»:
// в логе стояло 17:17, потом 20:17 -- и ни одной строки в промежутке, хотя копия честно
// обновлялась каждый час. Из лога это было не-отличимо от «таймер не работает».
// ============================================================================
...
function configSanityIssues ()
{
    ...
    // [v2.83] Статус канала: мусор в ключе -- это «не трогаю» (умолчание), а не «пишу»:
    // затирать чужую строку в шапке из-за опечатки нельзя.
    ...
    // [v2.89] ...и экземпляры, которые настроены, но не работают (см. serverServeIssues).
...
function configDriftIssues ()
{
    ...
    // [v2.88] ПОДСКАЗКИ -- ЭТАЛОН В ПРИМЕРЕ (правило владельца: «КОММЕНТАРИИ к боевому РАВНЫ
    // комментариям в example»). Раньше это проверялось глазами и разъезжалось: подсказка
    // в примере правилась, а в боевом файле оставалась старая -- и человек читал у себя в
    // конфиге неправду (например «по умолчанию false», когда это давно не так).
    // Сравниваем ТЕКСТЫ подсказок у ключей, которые есть в ОБОИХ файлах: там, где ключа
    // в боевом нет, сравнивать нечего (подсказка без ключа читалась бы как мусор).
...
// ============================================================================
// [v2.89] ЭКЗЕМПЛЯР ДОЛЖЕН РАБОТАТЬ -- А НЕ РАБОТАЕТ. После v2.85 включённым считается
// только allow: true, и забытая строка выглядит как «бот стоит в сервере, а ничего не
// делает». Самая частая причина именно такая: блок скопировали из примера (каналы и роли
// заполнены), а ключ allow не поставили. Об этом бот теперь говорит вслух -- и здесь
// (без Discord), и сразу после входа (там же видно и обратное: allow: true, а бота
// в этом сервере уже нет -- его выгнали или ни разу не пригласили).
// ============================================================================
const SERVER_TUNED_KEYS = ['name', 'log_channel', 'pipe_channel_source', 'pipe_channel_target',
    ...
    'bans_history_days', 'queue_page', 'addTag', 'tag'];
function serverServeIssues ()
{
    ...
    for (const id of Object.keys (SERVERS || {}))
    {
        ...
        const s = SERVERS[id];
        if (s.allow !== undefined) continue;         // явное true/false -- решение владельца
        const tuned = SERVER_TUNED_KEYS.filter (k => s[k] !== undefined);
        if (!tuned.length) continue;                  // пустой блок -- ничего не обещает
...
// Вторая половина -- по списку серверов, которые видит бот (только после входа).
...
(
    ...
    async () =>
    {
        ...
        // [v2.89] А теперь сверяем список серверов с конфигом: allow: true, а бота в сервере
        // нет -- или бот в сервере, а экземпляра нет. Без этого оба случая выглядят как
        // «бот молчит», и причину ищут в музыке и правах. Серверы видны только тут.
...
//   "channel_status": true -- бот пишет в шапку свою строку и снимает её при выходе;
//   false -- не трогает шапку вообще (ни пишет, ни снимает), и вся информация остаётся
//   в ПРОФИЛЬНОМ статусе бота («Слушает: … · в очереди 2 …»), его видно везде, даже вне
//   голосового.
// [v2.83] ПО УМОЛЧАНИЮ -- ВЫКЛЮЧЕНО (было включено до этой версии). Причина в самих
// настройках: `config.minimal.json` собран без блока MUSIC, и по старому умолчанию любой
// свежий бот молча затирал строку, которую поставил АВТОР канала, -- а вернуть её нельзя
// (роут только пишущий). Лишнее поведение включается одним ключом: "channel_status": true.
...
// [v2.91] Сколько сообщений /queue вести одновременно (каждое -- своя страница и свой
// зритель). Объявлено здесь, рядом с QUEUE_LIVE_MS: значение нужно уже в стартовой
// строке про самообновление, а до блока живых сообщений (ниже) дело дойдёт позже.
...
// [v2.61] Одна строка при старте: обновляются ли сообщения /queue сами и как часто.
// Раньше про это молчали, и «не обновляется» ничем не отличалось от «выключено».
// [v2.91] Сказано и про несколько сообщений: ведём до QUEUE_LIVE_MAX сразу.
    ...
    if (musicNormalizeReady || seekInFfmpeg)
    {
        ...
        if (ff)
        {
            ...
            ff.on ('close', code =>
            {
                ...
                // [v2.92] ЧЕСТНАЯ ФОРМУЛИРОВКА. Раньше здесь стояло «звук этого трека оборвался:
                // до конца он не доиграет, очередь продолжаю со следующего» -- читалось как (а)
                // «виновата громкость» и (б) «трек потерян» (владелец: «звучит фатально, даже
                // правдой не оказалось»). На самом деле loudnorm у нас ПОСЛЕДНИЙ в цепочке
                // yt-dlp -> ffmpeg, поэтому обрыв потока ругает именно он, а трек после этого
                // ещё пробуют продолжить с той же секунды (до MUSIC_STREAM_RETRIES раз) --
                // «к следующему» бот идёт только по строке «поток обрывается снова».
...
// ============================================================================
// [v2.89] ПАЧКА НЕ РАЗРЫВАЕТСЯ, А ИГРАЮЩИЙ ТРЕК -- ЕЁ НУЛЕВОЙ ЭЛЕМЕНТ.
// Владелец («Нельзя допускать разрыва пачек... Играемый трек считается 0-вым элементом,
// позицией в плейлисте и в пачке!») описал случай, который и случился в живом эфире:
// играл длинный сет одного автора, второй DJ поднял свои треки наверх -- и оказался
// ВНУТРИ чужой пачки: играющий трек, чужие треки, играющий трек... Плейлист читался
// рвано, а 0-й элемент (то, что играет) вообще не был позицией: его нельзя было выбрать.
//
// Теперь так: ПОЛНЫЙ ПЛЕЙЛИСТ -- это №0 (играющий трек) + №1..№N (очередь), и треки
// одного автора идут в нём ПОДРЯД. Значит если играет трек Фантазёра, то и остальные
// его треки должны стоять сразу за ним, а не через чужие. Через эти три функции проходят
// все перемещения (queueMove/queuePromote/queueDemote/queuePush/jumpMusic) -- чтобы
// правило было одно, а не своё в каждой команде.
// ============================================================================
// Автор элемента как ключ пачки ('': треки без автора -- тоже своя пачка, одной кучей).
function qKey (t) { return byIdOf (t); }

// Сколько ПОДРЯД идёт треков этого автора, начиная с позиции i списка.
...
// Начало пачки этого автора в списке (сколько треков стоит перед ней). Если своих треков
// в списке нет -- вернёт позицию ЗА концом: такая пачка встанет последней и никого не
// разорвёт (перед ней -- конец списка, а не чужая середина).
...
// Разорвёт ли вставка трека автора key в позицию idx списка ЧУЖУЮ пачку? Слева может
// стоять играющий трек (idx === 0 и он есть): он и есть №0 плейлиста, то есть часть
// своей пачки -- поэтому левый ключ передаётся отдельно (leftKey).
function qWouldSplit (list, idx, key, leftKey)
{
    ...
    if (left === null || right === null) return false;   // край плейлиста ничего не рвёт
    if (left !== right) return false;                    // это граница пачек -- встать можно
...
// [v2.89] ПАЧКА ИГРАЮЩЕГО ТРЕКА -- НАВЕРХ, ЕСЛИ ЕЁ РАЗОРВАЛО. Очередь могла быть разорвана
// ДО этой версии (как в живом эфире владельца: играл сет одного автора, его же треки
// уехали вниз, а чужие встали сразу за играющим). Все операции, которые «добавляют к
// началу» или ищут место для новых треков, сначала зовут эту функцию -- тогда они не
// встанут ВНУТРЬ чужой пачки. Возвращает true, если порядок действительно поменялся
// (вызывающие могут сказать об этом в ответе; делать что-то -- не обязаны).
...
// Готовое объяснение отказа: одинаковое во всех командах, чтобы не разъезжалось.
function qSplitText (key, extra)
{
    return '🚫 **Пачку разрывать нельзя.** В этом месте подряд идут треки одного автора (' +
        ...
        '_Поставь его на границу пачек (перед всей пачкой или после неё) либо подними всю ' +
        'свою пачку: `/push`._';
}
...
async function saveMusicState (guildId)
{
    try
    {
        ...
        await db (guildId, 'musicState', 'queue',
        {
            ...
            // [v2.57] Какие сообщения /queue ведут самообновление (чтобы оно пережило
            ...
            // перезапуска текст пуст, поэтому первая же проверка обновит сообщения.
            // [v2.91] Поле то же (qMsg), но теперь в нём СПИСОК: старые базы с одним
            // объектом читаются как раньше (см. queueWatchRestore).
...
async function historyReAdd (guildId, at, userId, byName, inCh)
{
    ...
    // [v2.89] ...и сначала -- порядок: пачка ИГРАЮЩЕГО трека склеивается, иначе новые
    // треки могли бы встать внутрь неё.
    qGluePlaying (m);
...
async function historyReAddOne (guildId, keyId, userId, byName, inCh)
{
    ...
    t.addAt = addedAt;
    t.addIn = inCh || null;
    qGluePlaying (m);   // [v2.89] порядок в очереди -- до того, как искать место
    const insAt = authorBlockInsertAt (m.tracks, userId);
...
// «...и ещё N» [v2.91: без подсказки про /queue from:M -- так хвост стал только короче]
// и РАЗДЕЛИТЕЛИ блоков [v2.32] (до трёх линий QSEP по 12 символов).
...
function queueListBudget (m)
{
    ...
    // [v2.92] маршрут теперь стоит в нижней сводке, но из бюджета никуда не делся,
...
function queuePage (m, start)
{
    ...
    // столько, сколько влезло; на следующую страницу ведут кнопки листания (шаг --
    // по показанному числу строк), поэтому потерянного куска между страницами нет.
...
function queueComponents (page, m, moveSel = 0, opts = {})
{
    ...
    (
        ...
        (
            ...
            // [v2.68] «⤴ Другой трек» (до этого «⤴ Перепрыгнуть» в третьем ряду; просьба
            // владельца: «перенеси во второй ряд в конец, назови "Другой трек"») -- это
            // ДЕЙСТВИЕ С ИГРАЮЩИМ треком, поэтому ему и место рядом с перемоткой и
            // «Пропустить». Пятая кнопка в ряду -- ровно предел Discord. Логика -- та же
            // /jump (список треков и подтверждение варианта, см. jumpPickerRows).
            new ButtonBuilder ()
    ...
    if (moveSel >= 1 && moveSel <= total)
    {
        ...
        (
            ...
            (
                ...
                // [v2.89] «▶ Играющим» -- поставить выбранный трек на №0: он заиграет сразу,
                // а его пачка поедет наверх целиком (см. queuePromote).
                new ButtonBuilder ()
...
//     возвращается в очередь со своего места (\"ничего не теряется\"). Что стоит до
//     цели -- не трогается; в конец ничего не уезжает.
//     [v2.89] Считается это как «поставить цель на №0» -- ровно то же делает кнопка
//     «▶ Играющим» и `/move to:0` (общая функция queuePromote). Заодно она бережёт
//     ПАЧКИ: пачка выбранного трека поднимается целиком, а прерванный встаёт в начало
//     СВОЕЙ пачки (жёсткое «следующим» ломало бы чужие пачки).
...
function jumpMusic (guildId, n, cut, opts = {})
{
    ...
    // [v2.63] СРОЧНЫЙ ПЕРЕХОД -- это «поставить цель на №0»: выбранный трек играет сразу,
    // а прерванный возвращается в очередь, ничего не теряя.
    // [v2.89] Логика теперь ОДНА с кнопкой «▶ Играющим» и `/move to:0` (queuePromote) --
    // и она же следит за пачками: пачка выбранного трека поднимается ЦЕЛИКОМ, а
    // прерванный встаёт в НАЧАЛО СВОЕЙ пачки (а не жёстко «следующим» -- иначе он
    // оказался бы посреди чужой пачки, а пачки у нас не разрываются).
...
function jumpConfirm (n)
{
    ...
    return {
        text: '⤴ **Перейти к №' + n + '?** Что сделать с тем, что стоит до него:\n' +
            // [v2.89] «Следующим» больше не обещаем: пачки не разрываются, поэтому прерванный
            // трек встаёт в начало СВОЕЙ пачки (а его треки в очереди обычно и стоят рядом
            // с ним) -- это и есть «следующим» в обычном случае и без разрыва чужой пачки.
...
// (иначе ответы и лог со временем разъехались бы). Номера те же, что в /queue, и с
// [v2.89] среди них есть №0 -- ЭТО ИГРАЮЩИЙ ТРЕК (владелец: «играемый трек считается
// 0-вым элементом, позицией в плейлисте и в пачке»). Возвращает { ok, text, to }.
function queueMove (guildId, n, to, who, opts = {})
{
    const m = musicOf (guildId);
    const total = m.tracks.length;
    // [v2.89] №0 -- такое же место плейлиста, как и остальные: поставить НА него = этот
    // трек начинает играть (а играющий уходит в очередь), снять С него = играющий
    // возвращается в очередь. Кто и когда это может -- те же права, что у перехода.
    // Проверки диапазона у них свои (у одного -- очередь, у другого -- играющий трек).
...
// ============================================================================
// [v2.89] «НА ПОЗИЦИЮ №0» -- выбранный трек НАЧИНАЕТ ИГРАТЬ (кнопка «▶ Играющим»,
// /move number:N to:0 и «срочный переход» из /jump). Раньше 0-го элемента не было:
// играющий трек не был позицией плейлиста, и перебить его можно было только переходом.
// Теперь это обычное место:
//   * цель играет сразу и уходит из очереди;
//   * ПАЧКА цели едет наверх ЦЕЛИКОМ -- иначе №0 разорвал бы её (её же треки остались
//     бы внизу, за чужими);
//   * прерванный трек встаёт в НАЧАЛО СВОЕЙ пачки и помнит секунду, на которой оборвался
//     (своих треков в очереди нет -- уходит в конец: там он никого не разорвёт).
// Права -- те же, что у «срочного перехода» (jumpGuard): DJ -- по своему треку, чужой
// играющий перебивают только админы и модеры.
// ============================================================================
function queuePromote (guildId, n, who, opts = {})
{
    ...
    const run = without.filter (t => qKey (t) === key);   // остальные треки ЭТОГО автора
    const rest = without.filter (t => qKey (t) !== key);
    const after = run.concat (rest);                      // его пачка -- наверх, целиком
    if (was)
    {
        was.seek = wasAt;                                 // место в треке не теряется
        after.splice (qRunHead (after, qKey (was)), 0, was); // в начало СВОЕЙ пачки
    }
    ...
    console.log ('[' + (d()) + '] [music] ' + whoText (who) + 'поставил играющим №' + n + ': ' + (target.title || 'трек') +
        ...
        ' -- это №0 плейлиста');
    if (was) { m.skipRequested = true; if (m.player) m.player.stop (true); } // Idle-хэндлер запустит цель
    else playNext (guildId);
    return {
        ok: true,
        to: 0,
...
// ============================================================================
// [v2.89] СНЯТЬ ИГРАЮЩИЙ С ЭФИРА (n === 0: «переставить №0»): трек возвращается в очередь
// С ТОГО ЖЕ МЕСТА, а играть начинает следующий. Место можно назвать -- но только на
// границе пачек: внутрь чужой пачки трек не встанет (она разорвётся). Своих треков
// в очереди нет -- без места он уйдёт в конец (так его пачка останется целой).
// Права -- как у /skip: свой трек снимает его автор, чужой -- только staff.
// ============================================================================
function queueDemote (guildId, to, who, opts = {})
{
    ...
    const key = qKey (was);
    // Играть начнёт первый трек очереди -- он встаёт на №0, а снятый уходит в очередь
    // (по умолчанию -- в НАЧАЛО СВОЕЙ пачки, там его и ждали его же треки).
    ...
    // Это осознанный уход вперёд, а не «поток умер»: трек уже в очереди, второй раз его
    // не кладём (поэтому и skipRequested -- тот же признак, что у /skip и /jump).
...
function queuePush (guildId, targetId, who)
{
    ...
    // [v2.89] ПАЧКА НЕ РАЗРЫВАЕТСЯ. Верх плейлиста занимает ИГРАЮЩИЙ трек (он №0), так
    // что «наверх» -- это СРАЗУ ЗА НИМ, а не перед ним (раньше /push ставил чужие треки
    // между играющим треком и остальными треками его автора и рвал его пачку).
    const curSame = !!m.current && same (m.current);
    const curKey = m.current ? qKey (m.current) : null;
    const rest = m.tracks.filter (t => !same (t));
    // Пачка играющего трека идёт сразу за ним. Если её когда-то разорвало -- здесь она
    // склеивается (владелец: «играемый трек должен был вместиться вверх остальных его
    // треков»): иначе наше «наверх» встало бы внутрь неё.
    let rest2 = rest;
    let glued = false;
    if (m.current)
    {
    ...
    // Играющий трек -- не наш: встаём сразу за его пачкой (она начинается с №0, то есть
    // с самого играющего); играет наш же автор -- мы и есть его пачка, значит в начало.
    ...
    if (m.current)
    {
        ...
        const pos = m.current.isLive
            ...
            : (m.current.duration > 0
                ...
                : '');
        // [v2.89] №0 у шапки не для красоты: играющий трек -- такое же место плейлиста,
        // как и остальные (на него можно ставить и с него можно снимать через /move,
        // и там же стоит его пачка), поэтому и номер у него свой.
...
// [v2.92] Эта строка -- единственная сетевая ВВЕРХУ /queue: она появляется, лишь когда
// музыка уже встала, а маршрут («чем идём, кто отдыхает») переехал в нижнюю сводку.
...
// -- тоже молчим.
// [v2.92] В /queue строка стоит в НИЖНЕЙ сводке (у владельца внизу сообщения), а в
// /nowplaying -- там же, где и раньше, после строки про канал.
...
function queueView (m, start, moveSel = 0, opts = {})
{
    ...
    // [v2.44] ПОРЯДОК: шапка -> справка (мелким) -> подсказка -> список.
    // [v2.91] И ПОСЛЕ списка -- сводка (по авторам, остаток, проверка): она
    // информационная, а не рабочая, и её место внизу.
    ...
    // [v2.92] Маршрут (чем идёт звук, кто отдыхает) -- вниз, к сводке: это такая же
    // справка, как «по авторам», и вверху оно мешало (владелец: «тоже перенести!»).
    // Вверху остаётся только netWaitText -- «почему тишина»: эта строка появляется,
    // лишь когда музыка уже встала, и объясняет, что происходит. Маршрут видит только
    // владелец хостинга (проверка внутри netRouteText).
    const net = netWaitText (m);
    const route = netRouteText (m.guildId, opts.actorId);
    const wait = queueWaitText (m);
    // [v2.91] СВОДКА -- В САМОМ НИЗУ, после списка. Владелец: «этот блок в САМЫЙ НИЗ,
    // он разрывает контроль и работу с очередью; я постоянно думаю, что играет №1,
    // а не №0». Поэтому сверху остаётся только рабочее (состояние сети, пачки, перенос),
    // затем очередь, а цифры (по авторам, до конца, проверка заранее, маршрут) -- в конце,
    // над кнопками (кнопки Discord всегда рисует ПОД embed'ом, ближе к низу не поставить).
    const build = hint => queueHeadText (m) +
        '\n' + QSEP + '\n' +
        [net, packList, move, hint].filter (Boolean).join ('\n') +
        '\n' + QSEP +
        '\n**Очередь (' + total + ')**' + (page.start > 1 ? ' · с №' + page.start : '') + ':\n' + page.list +
        // [v2.91] «...и ещё N» без подсказки про /queue from:N (владелец: «помощь по
        // команде там не нужна»). Дальше ведут кнопки листания внизу сообщения.
        (restNow () > 0 ? '\n*...и ещё ' + restNow () + '*' : '') +
        '\n' + QSEP + '\n' +
    ...
    else
    {
        ...
        // сообщения говорит, сколько осталось, а на следующую страницу ведут кнопки.
...
// ============================================================================
// [v2.57] СООБЩЕНИЯ /queue ОБНОВЛЯЮТСЯ САМИ, ПОКА ИХ ВИДЯТ.
...
// листать или нажать «🔄 Обновить». Теперь бот помнит сообщения, которые сам отправил
// (m.qMsgs), и раз в QUEUE_LIVE_MS собирает ту же СТРАНИЦУ заново из живой очереди --
// только если текст реально изменился (одинаковая правка всё равно считалась бы правкой).
//
// [v2.91] ВЕДЁМ НЕ ОДНО СООБЩЕНИЕ, А НЕСКОЛЬКО (до QUEUE_LIVE_MAX). Пока живая запись
// была одна, второе открытое /queue вытесняло первое: открыл с другого аккаунта -- и
// первое замирало, пока его не обновят вручную (владелец: «автоматически обновляется
// только самый последний»). Теперь у каждого сообщения своя запись -- канал, страница,
// зритель и время следующей правки, -- и обновляются все сразу.
//
...
// сообщения в нём). Наш шаг -- полминуты: даже четыре сообщения в одном канале дают
// 8 правок в минуту против допустимых ~60, и это без учёта того, что неизменившийся
// текст не правится вовсе. Дополнительно:
//   * пока в сообщении открыт перенос/меню автора, самообновление молчит -- иначе
...
//     ЭТО сообщение -- дальше в этом канале нет ни правок, ни запросов;
//   * сообщение удалили или к нему нет прав (10008, 50001, 403...) -- молча перестаём
//     его вести; а вот ВРЕМЕННЫЙ сбой сети правку не убивает: запись остаётся, и
//     следующая попытка доводит обновление до конца (владелец видел в логе Connect
//     Timeout -- после него сообщение замирало до ручного нажатия);
//   * 0 в конфиге выключает самообновление целиком (таймер даже не заводится).
// Первая самоправка КАЖДОГО сообщения пишется в лог одной строкой -- по ней видно, что
// оно действительно работает (дальше молча: 30 строк в час -- это уже спам, не лог).
// ============================================================================
const QUEUE_LIVE_STEP = 5000;   // как часто проверяем, кому пора обновиться (5 с)
// (QUEUE_LIVE_MAX объявлен выше, рядом с QUEUE_LIVE_MS -- он нужен уже в стартовой
// строке про самообновление.)

// Запись живого сообщения по его id (или null).
...
// Перестать вести ЭТО сообщение (остальные записи не трогаем).
...
function queueWatch (m, msg, ctx, page, text, move)
{
    ...
    const rec = { ch: msg.channelId, id: msg.id, ctx: ctx, page: Math.max (1, Number (page) || 1),
                  ...
                  // [v2.61] первая самоправка пишется в лог -- видно, что оно работает.
                  // Уже знакомое сообщение не заводим заново (иначе сбрасывался бы first).
                  first: old ? !!old.first : true };
    if (old) m.qMsgs[m.qMsgs.indexOf (old)] = rec;
    else m.qMsgs.push (rec);
    // Помним не больше QUEUE_LIVE_MAX: забываем САМОЕ ДАВНО НЕ ТРОНУТОЕ (там уж точно
    // никто не смотрит).
...
// Восстановить живые сообщения из записи в базе (переживают перезапуск). Текст не
// берём -- его всё равно пересобираем из живой очереди на первой же проверке.
// [v2.91] В базах до этой версии лежал ОДИН объект -- понимаем и его.
...
// Сбой правки, после которого вести сообщение бессмысленно: его удалили, канал пропал,
// прав нет. Всё остальное (таймауты, 5xx, оборванное соединение) -- ВРЕМЕННОЕ:
// запись оставляем, и следующая попытка доведёт правку до конца.
...
// Одна перерисовка одного сообщения. НИКОГДА не бросает: сбой правки не должен
// ронять бота -- он только решает, забыть ли эту запись (queueWatchGone).
async function queueLiveOne (m, w)
{
    ...
    {
        if (empty) queueWatchForget (m, w.id);   // очередь доиграла -- и этот вид уже показан
        return;
    }
    try
    {
        ...
        // [v2.61] Первую самоправку видно в логе (дальше -- молча, иначе был бы спам).
        if (w.first) { w.first = false; console.log ('[' + (d()) + '] [music] сообщение /queue обновил сам (стр. ' + w.page + ')'); }
        w.text = content;
        w.err = false;                    // [v2.91] сеть снова отвечает -- сообщение живое
        if (empty) queueWatchForget (m, w.id);   // финальный вид показали -- больше не правим
    }
    catch (e)
    {
        if (queueWatchGone (e)) { queueWatchForget (m, w.id); return; }
        // [v2.91] Временный сбой (сеть, таймаут): сообщение НЕ бросаем -- про ошибку
        // скажем один раз на серию, а следующий проход (через QUEUE_LIVE_MS) обновит его.
        if (!w.err)
        {
            w.err = true;
...
// Проход по всем живым сообщениям сервера. НИКОГДА не бросает наружу.
...
async function queueMsgRedraw (guildId, delayMs = 0)
{
    ...
    // [v2.91] После действия обновляем ВСЕ ведомые сообщения: у каждого своя страница
    // и свой зритель, и каждому важно увидеть новую очередь сразу (иначе одно из них
    // дожидалось бы общего самообновления).
    for (const w of m.qMsgs.slice ())
    {
        try
        {
...
if (QUEUE_LIVE_MS)
    setInterval (() =>
    {
        ...
        for (const g of Object.keys ($music))
        {
            ...
            // Отмечаем ВСЕ записи сервера ещё до правки: правка асинхронная, а шаг
            // таймера -- 5 с, и без этого проход мог бы наложиться сам на себя.
...
function checkListeners (server, _noFollow = false)
{
    ...
    // [v2.73] КАНАЛА ИЗ ПРОШЛОГО ЗАПУСКА МОЖЕТ УЖЕ НЕ БЫТЬ -- И ЭТО НОРМАЛЬНО. Владелец:
    // «Бот должен возвращаться К СЛУШАТЕЛЮ, а не тупо в канал. Если канала нет или он
    // пустой -- слушателя там нет». Так и есть: личный временный канал убирается, когда
    // опустел (см. tempSweep), а очередь от этого не страдает. Но раньше вместе с каналом
    // пропадала и возможность прийти к людям: без savedChannelId функция уходила в return,
    // и бот ждал только /join, хотя автор трека мог уже сидеть в голосовом. Теперь ищем
    // СЛУШАТЕЛЯ -- им считается автор ближайшего нужного трека (pickAuthorChannel).
    // Никого из авторов в голосовых нет -- ехать некуда, ждём следующего события или /join.
    if (_noFollow) return;   // второй проход никого не догоняет (защита от беготни)
...
[
    ...
    new SlashCommandBuilder ()
        .setName ('move')
        .setDescription ('Переставить трек в очереди на другое место (0 -- это то, что играет)')
        .addIntegerOption (o =>
            o.setName ('number')
...
client.on ('interactionCreate', async (interaction) =>
{
    ...
    if (typeof interaction.isModalSubmit === 'function' && interaction.isModalSubmit ())
    {
        ...
        // [v2.89] №0 -- ИГРАЮЩИЙ трек (владелец: «играемый трек считается 0-вым элементом,
        // позицией в плейлисте и в пачке»), поэтому в окне принимаем и его.
    ...
    if (interaction.isButton () || interaction.isStringSelectMenu ())
    {
        ...
        // [v2.89] «▶ Играющим» -- выбранный трек встаёт на №0 и играет сразу (его пачка
        // едет наверх целиком). После этого он уже не в очереди, поэтому возвращаемся
        // к обычным кнопкам очереди, а не остаёмся в режиме переноса.
                ...
                new ModalBuilder ().setCustomId ('q:mpos:' + mPosAsk[1])
                    ...
                    (
                        ...
                        (
                            new TextInputBuilder ()
                                .setCustomId ('pos')
                                // [v2.89] В подписи видно и №0 -- так понятно, что играющий трек
                                // тоже место плейлиста; в поле уже стоит текущий номер (править
                                // пару цифр -- быстрее, чем вспоминать формат).
        ...
        // уезжал наверх. Теперь берём ту страницу, которую ведёт самообновление, если
        // речь именно об этом сообщении (m.qMsgs -- записей может быть несколько).
    ...
    const guildId = interaction.guildId;
    const m = musicOf (guildId);
    // [v2.67] ПОСЛЕ ДЕЙСТВИЯ КОМАНДОЙ СРАЗУ ОБНОВЛЯЕМ СООБЩЕНИЯ ОЧЕРЕДИ, которые бот ведёт
    // (m.qMsgs -- их может быть несколько): раньше их подхватывало только самообновление,
    // до 30 секунд, и в канале оставались старая очередь и старое «Сейчас» (владелец:
    // «после перепрыгивания embed сам мгновенно не обновился»). 1500 мс -- там, где плеер
    // перезапускается (прыжок, перемотка): сразу после них в строке «Сейчас» был бы ноль.
    ...
    try
    {
        ...
        if (name === 'play')
        {
            ...
            // [v2.89] Сначала -- порядок: пачка ИГРАЮЩЕГО трека склеивается (её могло
            // разорвать до этой версии), и только потом ищем место для новых треков.
            qGluePlaying (m);
        ...
        else if (name === 'queue')
        {
            ...
            // Сообщение не пришло (Discord не отдал его в ответе) -- вести это нечего, но
            // ОСТАЛЬНЫЕ ведомые сообщения не бросаем: их открывали другие люди, и они
            // должны обновляться дальше (раньше одна неудача обнуляла живое сообщение).
